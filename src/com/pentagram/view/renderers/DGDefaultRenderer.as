package com.pentagram.view.renderers
{
	 ...
	 */
	public class DGDefaultRenderer extends UIComponent implements IGridItemRenderer
	{
		public function DGDefaultRenderer()
		{
			super();
		...
		public function set singleLine(value:Boolean):void
		{
			...
			_singleLine = value;
			if(labelDisplay) {
				labelDisplay.multiline = false;//!singleLine;  // can be reset, see updateMeasuredSize()
				labelDisplay.wordWrap = false;//!singleLine;
			}
		}
		...
		protected function createLabelDisplay():void
		{
			...
			labelDisplay.multiline = false;  // consistent with singleLine=false (the default)
			labelDisplay.wordWrap = false;
			labelDisplay.autoSize = TextFieldAutoSize.NONE;
			labelDisplay.embedFonts = true;
		...
		private function updateMeasuredSize():void
		{
			if (getStyle("lineBreak") == "explicit")
				labelDisplay.multiline = false;//_label.indexOf("\n") != -1;
